...
class Arm_Device(object):
    ...
    #Set the bus servo neutral offset with one click, power on and move to the neutral position, and then send the following function, id: 1-6 (setting), 0 (restore to initialization)
    ...
    ## Read the status of the mid-bit offset of the one-click setting bus servo. 0 means that the corresponding servo ID cannot be found, 1 means success, and 2 means failure is out of range.
   ...
   # Read hardware version number
    ...
    #Set the current product color 1~6, the corresponding color when the RGB light turns on.
    ...
    # Action group running mode 0: Stop running 1: Single running 2: Cycle running
