...
# 看地图中心初始位置
joints_init0 = [90, 135, 0,0, 90, 0]
# 引蛇出洞
joints_init_snake = [90, 135, 0,45, 0, 180]
# 获取本机IP
def getLocalip():
...
#             g_colorstudyMode = 'colorstudyModeCancel'
        ...
        if mode == 1: #你放我抓
            if switch == 1:
                Arm.Arm_serial_servo_write6_array(joints_init0, 500)
                g_mode = 'YouputWecatchMode'
            else:
                g_mode = 'Standard'
        elif mode == 2: #引蛇出洞
            Arm.Arm_serial_servo_write6_array(joints_init_snake, 1500)
            if switch == 0:
                g_mode = 'Standard'
            elif switch == 1:
    ...
    while True:
        ...
        if g_colorstudyMode == 'colorstudyMode': #进入颜色学习模式
            ...
            g_color_dict =  {"red": ((0, 150, 160), (10, 255, 255)),
                          ...
                          "blue": ((116, 124, 100), (130, 255, 255)),
                          "yellow": ((20, 98, 130), (40, 255, 255))}
        elif g_colorstudyMode == 'colorstudyMode_HSV': #进入颜色校准模式 colorstudyMode_HSV
            hsv_msg = {}
            hsv_msg[g_color_hsv] = g_color_dict_temp[g_color_hsv]
            if g_colorDisplay == 'Normal':
                frame, _ = colorCalibration.get_contours(frame, g_color_hsv,hsv_msg[g_color_hsv],g_color_dict)
            else:
                _, frame = colorCalibration.get_contours(frame, g_color_hsv,hsv_msg[g_color_hsv],g_color_dict)
        elif g_colorstudyMode == 'colorstudyModeOK_HSV': #颜色校准确定
            if g_color_hsv!=None:
...
#             print(pos)
        ...
        # 颜色追踪玩法
            ...
            frame = Arm_color_follow.run(frame, 'red',g_color_dict['red'])
        elif g_mode == 'colortrack2':
            frame = Arm_color_follow.run(frame, 'green',g_color_dict['green'])
        elif g_mode == 'colortrack3':
            frame = Arm_color_follow.run(frame, 'blue',g_color_dict['blue'])
        elif g_mode == 'colortrack4':
            frame = Arm_color_follow.run(frame, 'yellow',g_color_dict['yellow'])
...
# wifi 配网的线程。
def thread_wifi():
    global g_camera, g_wifi

    frame = g_camera.get_frame()
    _, ip = Arm_wifi.connect(frame)
    while True:
        if g_wifi == True:
            frame = g_camera.get_frame()
            _, ip = Arm_wifi.connect(frame)
